...
namespace DoDo
{
	...
	int16_t FSlateFontRenderer::get_base_line(const FSlateFontInfo& in_font_info, const float in_scale) const
	{
		DoDoUtf8String Char("\0");//todo:fix me, use utf8 str
		...
		if (face_glyph_data.m_face_and_memory && face_glyph_data.m_face_and_memory->is_face_valid())
		{
			...
			//const float height_adjustment = in_font_info.m_outline_settings.m_outline_size * in_scale;
			return static_cast<uint16_t>(FreeTypeUtils::Convert26Dot6ToRoundedPixel<int16_t>(face_glyph_data.m_face_and_memory->get_descender()));
		}
