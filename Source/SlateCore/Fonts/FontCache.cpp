...
namespace DoDo
{
	...
	int16_t FCharacterList::get_base_line() const
	{
		if (m_base_line == 0)
		{
			m_base_line = m_font_cache.get_base_line(m_font_key.get_font_info(), m_font_key.get_scale());
		}
	...
	int16_t FSlateFontCache::get_base_line(const FSlateFontInfo& in_font_info, float font_scale) const
	{
		return m_font_renderer->get_base_line(in_font_info, font_scale);
	}
	...
	std::optional<int32_t> FShapedGlyphSequence::get_measured_width(const int32_t in_start_index, const int32_t in_end_index, const bool in_include_kerning_with_preceding_glyph) const
	{
		...
		if (enumerate_logical_glyphs_in_source_range(in_start_index, in_end_index, glyph_call_back) == EEnumerateGlyphsResult::EnumerationComplete)
		{
	...
	FShapedGlyphSequence::EEnumerateGlyphsResult FShapedGlyphSequence::enumerate_logical_glyphs_in_source_range(const int32_t in_start_index, const int32_t in_end_index, const FForEachShapedGlyphEntryCallback& in_glyph_call_back) const
	{
		...
		int32_t source_index = in_start_index;
		while (source_index < in_end_index)
		{
			//get the glyphs that correspond to this source index
			const FShapedGlyphSequence::FSourceIndexToGlyphData* source_inde_to_glyph_data = m_source_indices_to_glyph_data.get_glyph_data(source_index);
			if (!(source_inde_to_glyph_data && source_inde_to_glyph_data->is_valid()))
			{
				return EEnumerateGlyphsResult::EnumerationFailed;
			}

			//enumerate each glyph generated by the given source index
			const int32_t start_glyph_index = source_inde_to_glyph_data->get_lowest_glyph_index();
			const int32_t end_glyph_index = source_inde_to_glyph_data->get_highest_glyph_index();

			for (int32_t current_glyph_index = start_glyph_index; current_glyph_index <= end_glyph_index; ++current_glyph_index)
			{
				const FShapedGlyphEntry& current_glyph = m_glyphs_to_render[current_glyph_index];

				if (!in_glyph_call_back(current_glyph, current_glyph_index))
				{
					return EEnumerateGlyphsResult::EnumerationAborted;
				}

				//advance the source index by the number of characters within this glyph
				source_index += current_glyph.m_num_characters_in_glyph;
			}
		}

		return (source_index == in_end_index) ? EEnumerateGlyphsResult::EnumerationComplete : EEnumerateGlyphsResult::EnumerationFailed;
	}
}
